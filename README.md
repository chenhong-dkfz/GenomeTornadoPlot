...
Users can choose if they want to calculate the standard, the edge focallity socre or simply compute total size of events as the "focality score".
Please notice that the focallity score of each gene is calculated by the data you give, so it is not a constant value in different datasets.
...
3. **score.type**: if the value is "del", calculate focallity score of deletions.if the value is "dup", calculate focallity score of duplications.
4. **score.method**: if the value is "normal", calculate standard focallity score.if the value is "edge", calculate edge score. if the value is "simple", calculate total numbers of events.
